...
document.addEventListener('DOMContentLoaded', function () {
    ...
    arrows.forEach(function(arrow) {
        arrow.addEventListener('click', function() {
            let currentDir = parseInt(arrow.getAttribute('data-glide-dir').substring(1)); // Remove o "=" e converte para número
            const glide = document.querySelector('.glide').glide; // Obtém a instância Glide

            // Se for a seta esquerda (retroceder)
            if (arrow.classList.contains('glide__arrow--left')) {
                glide.go('=' + (currentDir - 4)); // Retrocede 4 slides
            } 
            // Se for a seta direita (avançar)
            else if (arrow.classList.contains('glide__arrow--right')) {
                glide.go('=' + (currentDir + 4)); // Avança 4 slides
            }
        });
    });
});
